...
4. **Удаление пустых строк:**
   * В процессе форматирования сервис удаляет все пустые строки (включая строки из одних пробелов и табуляций): и из результата AI-модели (пост-обработка `cleanAiHtmlOutput` / `cleanAiPlainOutput`), и в локальных типографических движках. Пустая строка в выводе сервиса превращается в видимую пустую строку в RichTextArea (`CandidateCVEdit` заменяет перевод строки на `<br>`), поэтому итоговый HTML и Plain Text всегда без пустых строк.
