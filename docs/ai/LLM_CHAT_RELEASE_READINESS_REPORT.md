...
- UI shell 8080 ранее открывался в браузере, но финальная проверка после reload получила `ERR_CONNECTION_REFUSED`; authenticated flow чата не подтвержден. Массовая проверка 20–50 UI-сессий исключена по распоряжению пользователя. Общий 8080 не перенастраивался намеренно; его текущая недоступность требует проверки владельца Antigravity. Production не затрагивался.
...
- Общий локальный 8080 в финальной проверке недоступен (`ERR_CONNECTION_REFUSED`); authenticated UI gate нельзя выполнить до восстановления его владельцем.
