...
3. **Markdown-рендеринг и санитизация HTML**:
   - Полная поддержка форматирования Markdown: жирный/курсивный текст, списки, таблицы, блоки кода с подсветкой синтаксиса (`pre.llm-md-pre`), цитаты и ссылки `hrm://`.
   - Санитизация HTML по строгому whitelist тегов (`b`, `i`, `span`, `table`, `code` и др.) с фильтрацией опасных скриптов и инлайновых обработчиков `on*`.
4. **Интеллектуальный резолвинг сущностей HRM и устранение кракозябр UUID (`HrmEntityNameResolver`)**:
   - Автоматическая замена технических UUID в выводе модели (например, `""05532e90-1b43-5a2f-47ad-8dca3dcb665a"`, `<code>UUID</code>` или одиночные идентификаторы) на интерактивные ссылки с человекочитаемым наименованием (`💼 Вакансия Middle System Analyst / Системный аналитик`, `👤 Кандидат ...`, `📄 Резюме ...`, `📋 Взаимодействие ...`, `🏢 Компания ...`).
   - Автоматическое вырезание префиксного визуального мусора перед ссылками: `<span style="..."><code>UUID</code></span> — ` или `<code>UUID</code> — `.
   - Нормализация ссылок, где текст ссылки является сырым UUID: подстановка названия сущности вместо идентификатора.
   - Автоматическое форматирование наименований вакансий (добавление префикса `Вакансия `, отсечение внутренних проектных скобок `(ДКС...)` / `(Юзтех...)`).
   - Защита HTML-атрибутов (`data-id="UUID"`, `href=...`) от случайных замен и экранирование скобок в наименованиях.
   - Потокобезопасный in-memory кэш (`ConcurrentHashMap`) с атомарным `computeIfAbsent` для исключения параллельной нагрузки на БД и 60-секундным TTL для отрицательного кэша (`NOT_FOUND_CACHE`).
   - Интерактивный переход в карточку сущности по клику через JavaScript-мост `hunttechOpenHrmEntity(...)`.
5. **Динамические статусы и визуализация live-генерации**:
   - Кастомные live-бейджи текущего этапа генерации («Анализ контекста...», «Выполняется запрос...», «Hermes обрабатывает запрос...»).
   - Анимированный пульсирующий курсор (`.llm-chat-cursor`).
   - Форматированные штампы даты и времени отправки и получения каждого сообщения (`dd.MM.yyyy HH:mm:ss`).
