...
1. **Обычный пользователь (Рекрутер / Исследователь / Собеседующий)**:
   ...
   - Вкладка «Hermes — управление HRM» — **скрыта** (`visible="false"`).
   - Вкладка «Hermes (Чтение HRM)» — **доступна** (read-only контур `hrm-viewer`).
2. **Менеджер / Директор (CUBA Role: `Manager`, caption: «Директор»)**:
   - Вкладка «Локальный чат» — **доступна**.
   - Вкладка «Hermes (Чтение HRM)» — **доступна**.
   - Вкладка «Hermes — управление HRM» — **доступна**.

### Миграция БД
Для роли `Manager` назначены права через SQL-миграцию:
