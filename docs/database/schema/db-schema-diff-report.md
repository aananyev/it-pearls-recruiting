...
> **Связанные документы:** [LOCAL_DATABASE.md](../../operations/local-development/local-database.md) · [user-settings-photo-sync.md](../../business-rules/user-settings-photo-sync.md) · [ExtUser](../../entities/ext-user/ExtUser.md)  
...
| Дата | Изменение |
|------|-----------|
| 2026-06-30 | Первичный отчёт: сравнение live БД `HuntTech` с entity-моделью и отложенными миграциями пакета `26` |
...
1. **Модель приложения (ожидаемая схема)** — Java-сущности в `modules/global/src/com/company/hunttech/entity/`, расширения CUBA (`ExtUser` → таблица `SEC_USER`), Liquibase-описание в `modules/core/db/changelog/`.
2. **Живая база PostgreSQL** — `HuntTech` на `127.0.0.1:5432`, пользователь `cuba`. Сверка через `information_schema` и журнал миграций CUBA `SYS_DB_CHANGELOG`.
...
| 2 | **AI-поля вакансии** — черновик описания, чеклист интервью, карта поиска, план интервью | `HUNTTECH_OPEN_POSITION` | `RAW_DESCRIPTION`, `INTERVIEW_CHECKLIST`, `SEARCH_MAP`, `INTERVIEW_PLAN` | Колонки отсутствуют | **Блокирует** сохранение AI-полей на форме вакансии | `260627-1-updateOpenPosition.sql` |
| 3 | **Персональные AI-настройки** — API-ключ и модель LLM для каждого пользователя | `HUNTTECH_USER_AI_CONFIGURATION` | Таблица целиком (7 бизнес-колонок + стандартные CUBA) | Таблица отсутствует | **Блокирует** экран настройки AI пользователя | `260627-2-createUserAiConfiguration.sql`, `260627-4-updateUserAiConfiguration.sql` |
| 4 | **Шаблоны промптов для вакансий** — справочник текстов для AI | `HUNTTECH_VACANCY_PROMPT_TEMPLATE` | Таблица целиком (`CODE`, `NAME`, `PROMPT_TEXT`, …) | Таблица отсутствует | **Блокирует** CRUD шаблонов промптов | `260627-3-createVacancyPromptTemplate.sql`, `260627-4-updateUserAiConfiguration.sql` |

### Уже в синхронизации (миграция не нужна)

| Область | Таблица | Статус |
|---------|---------|--------|
| Персональные настройки (legacy-фото) | `HUNTTECH_USER_SETTINGS` | Колонка `IMAGE_ID` есть — соответствует `UserSettings.fileImageFace` |
| Трудовые договоры | `HUNTTECH_LABOR_AGREEMENT` | Все поля entity присутствуют |
| Фото кандидата | `HUNTTECH_JOB_CANDIDATE` | Поле `FILE_IMAGE_FACE` / связи в порядке |
| Связи вакансия ↔ договор | `HUNTTECH_OPEN_POSITION_LABOR_AGREEMENT_LINK` и др. | Join-таблицы существуют |
...
| `DTYPE` в `HUNTTECH_OPEN_POSITION` | Legacy-дискриминатор CUBA в БД, не объявлен в entity — не блокер |
...
Подробнее: [user-settings-photo-sync.md](../../business-rules/user-settings-photo-sync.md).
...
1. Добавляет 4 AI-колонки в `HUNTTECH_OPEN_POSITION`.
2. Создаёт таблицы `HUNTTECH_USER_AI_CONFIGURATION` и `HUNTTECH_VACANCY_PROMPT_TEMPLATE` с индексами и FK.
...
# 1. Бэкап
pg_dump -h 127.0.0.1 -U cuba -d HuntTech -Fc -f backup-before-updateDb.dump
...
-- AI-колонки вакансии
SELECT column_name FROM information_schema.columns
WHERE table_name = 'hunttech_open_position'
...
| Риск | Митигация |
|------|-----------|
| БД в режиме recovery (read-only после restore) | Создать чистую БД — см. [LOCAL_DATABASE.md](../../operations/local-development/local-database.md) § «recovery» |
