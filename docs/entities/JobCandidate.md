...
| Вкладки | Первый заход → загрузка справочников, generators, scan CV |
...
| Дата | Изменение |
|------|-----------|
| 2026-06-27 | Browse `fileImageFace`: круглая миниатюра 30×30 (`candidate-face-thumb`), hover-preview 300×300 через HTML `descriptionProvider` + `/dispatch/download?f=` (`FileDescriptorImageHelper`) |
