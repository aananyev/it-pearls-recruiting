...
> Living-doc: [OpenPosition.md](OpenPosition.md)
...
1. PostgreSQL 11+ локально (см. [LOCAL_DATABASE.md](../../operations/local-development/local-database.md)): БД `HuntTech`, пользователь `cuba`/`cuba`.
2. Создание схемы:
   ```bash
   ./gradlew createDb
   ./gradlew updateDb
