...
- Создание из карточки пользователя → родительский `ExtUser` известен → владелец назначается до открытия/commit редактора.
- Создание из личных настроек → текущий пользователь известен → владелец назначается до открытия/commit редактора.
- Создание из общего реестра → используется пользователь текущей сессии → `USER_ID` назначается до commit.
- Commit без владельца → связь `user` отсутствует → commit блокируется понятным сообщением; DB `NOT NULL`/FK остаются последней линией защиты.
- Выполнение AI-функции → найдена активная персональная конфигурация → сервис использует её согласно политике маршрутизации.
...
`UserAiConfigurationEdit` — lookup провайдеров, автопривязка `user` вызывающим экраном и обязательная pre-commit проверка владельца. `createdBy` является только полем аудита и не используется для восстановления FK.
