...
Для анализа навыков поле `executionSource` отделяет реальный AI-вызов от словарного fallback и неполных metadata. Для корпоративного или персонального AI-подключения мониторинг сохраняет фактически использованные `providerCode` и `modelName`. Fallback не выдаётся за модель: мониторинг показывает «Fallback: справочник». Для исторических строк, созданных до появления metadata, используется «Метаданные недоступны» без ретроактивной подстановки провайдера или модели.
