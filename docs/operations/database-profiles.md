...
Для JNDI/XML-шаблонов runtime-переменная пароля называется
`HUNTTECH_DB_PASSWORD`. `scripts/start-app.sh` экспортирует её из профильной
переменной только внутри процесса запуска. Production runtime должен получить
`HUNTTECH_DB_PASSWORD` из secret store; значение в Git не хранится.

Для IDE/Jetty задайте также `HUNTTECH_DB_JDBC_URL` и `HUNTTECH_DB_USER` из того
же профиля; `jetty-env.xml` читает их через `System.getenv` и не содержит
адресов окружений по умолчанию.
