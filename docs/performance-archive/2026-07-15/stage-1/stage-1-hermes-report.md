...
| Фото, рейтинг, контакты отображаются | ✅ | |
| Вкладка Взаимодействия — строки загружаются | ✅ | |
| Фильтр по вакансии работает | ✅ | |
| Повторное открытие — данные не дублируются | ✅ | |
| Создание/редактирование/удаление взаимодействия | ✅ | |
| Сохранение карточки кандидата | ✅ | |
| Новый кандидат — создание работает | ✅ | |

**Лог-анализ:** за проверочный период не обнаружено ошибок, связанных с Stage 1. Pre-existing: 11 строк `No description for entity` (FTS), 4 строки `NullPointerException: sendingMessage.caption` (email).
