...
## 4. Стилистика и компоновка интерфейса

### Улучшенная компоновка экранов:
1. **Информационный баннер заголовка (`ai-header-card`)**:
   - Крупный заголовок с пиктограммой и поясняющим подзаголовком.
   - Цветной чип-бейдж роли (`ai-chip-badge`): «Личный кабинет» для рекрутера / «Администрирование» для администратора.
2. **Оптимизированная панель фильтрации (`ai-dashboard-filter-bar`)**:
   - Четкое вертикальное и горизонтальное выравнивание инпутов с аккуратными подписями.
   - Двухрядная структура в панели администратора (1-й ряд — период и даты, 2-й ряд — сотрудник, провайдер, тип ключа и кнопки действий).
3. **Адаптивная сетка KPI-карточек (`ai-kpi-grid` / `ai-kpi-card`)**:
   - Карточки с цветной левой полосой-акцентом (Blue, Purple, Green, Orange) и мягким градиентом.
   - Иконка в заголовке метрики, крупное значение (24px bold) и компактная вторичная метрика.
4. **Вкладки для админ-панели (`ai-dashboard-tabsheet`)**:
   - Разделение на **«Графики и аналитика»** (сетка графиков 2x2 повышенной высоты) и **«Сводный реестр пользователей»** (полноразмерная таблица пользователей с детализацией).
5. **Таблицы данных (`ai-dashboard-table`)**:
   - Стилизованные строки с эффектом подсветки при наведении (`hover`) и скруглением границ карточки.
