...
| Дата | Изменение |
|---|---|
| 2026-08-14 | §6: CityEdit sidebar доведён до эталона ProjectEdit (серия гео-форм) — стандартные отступы контента sidebar (`padding: 14px 16px 12px` + граница + тень), подпись типа записи «Город» удалена из identity, название города по центру по горизонтали, footer-кнопки ОК/Отмена в правом нижнем углу со стилем primary/secondary (`city-editor-primary-action`/`city-editor-secondary-action`); сводка `edit-sidebar-summary` с подписью «Регион» сохранена; тест `GeolocationEditFormsContractTest` дополнен |
