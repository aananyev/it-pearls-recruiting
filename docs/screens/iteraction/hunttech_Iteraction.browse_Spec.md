# Iteraction Browse (`hunttech_Iteraction.browse`)
...
Контроллер `hunttech_Iteraction.browse`; навигация и дочерние формы — §3 «Иерархия и взаимосвязь форм».
...
| **Java-класс** | `com.company.hunttech.web.screens.iteraction.IteractionBrowse ` |
...
| **Меню** | `web-menu.xml` → `screen="hunttech_Iteraction.browse"` (если есть пункт) |
...
```
select e from hunttech_Iteraction e order by e.number, e.iterationName
```
