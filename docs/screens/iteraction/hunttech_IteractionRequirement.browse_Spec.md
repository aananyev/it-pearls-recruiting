# Iteraction Browse (`hunttech_IteractionRequirement.browse`)

> Сущность: [Iteraction.md](../../entities/iteraction/Iteraction.md)
...
Контроллер `hunttech_IteractionRequirement.browse`; навигация и дочерние формы — §3 «Иерархия и взаимосвязь форм».
...
| Параметр | Значение |
|----------|----------|
| **@UiController** | `hunttech_IteractionRequirement.browse` |
| **Java-класс** | `com.company.hunttech.web.screens.iteraction.IteractionRequirementBrowse ` |
| **XML-дескриптор** | `iteraction-requirement-browse.xml` |
| **messagesPack** | `com.company.hunttech.web.screens.iteraction` |
| **Базовый класс** | `StandardLookup` |
| **Lookup-компонент** | `iteractionsTable` |
| **EditedEntityContainer** | `` |
| **focusComponent** | `iteractionsTable` |
| **Меню** | `web-menu.xml` → `screen="hunttech_IteractionRequirement.browse"` (если есть пункт) |
...
```
select e from hunttech_Iteraction e order by e.number
```
...
| Парный экран | `hunttech_Iteraction.edit` | create / edit action |
...
| Элемент | Источник |
|---------|----------|
| Caption | `msg://` / `mainMsg://` из `com.company.hunttech.web.screens.iteraction` |
