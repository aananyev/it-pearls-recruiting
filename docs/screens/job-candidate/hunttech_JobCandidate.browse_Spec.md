...
| **Импорт резюме (Quick Load)** | Меню `quickLoadCV`: парсинг PDF (извлечение лиц), DOC/DOCX, парсинг текста из буфера обмена | Кнопка «Умная загрузка» (`smartUploadBtn` / `SmartCvUploadScreen`) с дедупликацией, авто-фокусом, скроллом к созданному кандидату и синхронизацией сайдбара + меню `quickLoadCV` (PDF, Word, буфер) |
...
| Дата | Изменение |
|------|-----------|
| 2026-09-23 | BL-2026-024: В `JobCandidateReestr` после завершения мастера «Умная загрузка» (`SmartCvUploadScreen`) обеспечен сброс фильтра меток (`clearSignIconFilter()`), установка области на «Все кандидаты» (`ALL`), догрузка кандидата в `jobCandidatesDc` при выпадении из лимита первых 200 строк, выбор строки (`setSelected`), прокрутка (`scrollTo`), фокус таблицы и мгновенная синхронизация сайдбара профиля (`populateDetailPane`). |
