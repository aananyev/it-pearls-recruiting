# OpenPositionComment Browse (`hunttech_OpenPositionComment.browse`)

> Сущность: [OpenPositionComment.md](../../entities/open-position-comment/OpenPositionComment.md)
...
Контроллер `hunttech_OpenPositionComment.browse`; навигация и дочерние формы — §3 «Иерархия и взаимосвязь форм».
...
| Параметр | Значение |
|----------|----------|
| **@UiController** | `hunttech_OpenPositionComment.browse` |
| **Java-класс** | `com.company.hunttech.web.screens.openpositioncomment.OpenPositionCommentBrowse ` |
| **XML-дескриптор** | `open-position-comment-browse.xml` |
| **messagesPack** | `com.company.hunttech.web.screens.openpositioncomment` |
...
| **Меню** | `web-menu.xml` → `screen="hunttech_OpenPositionComment.browse"` (если есть пункт) |
...
```
select e from hunttech_OpenPositionComment e
```
...
| Парный экран | `hunttech_OpenPositionComment.edit` | create / edit action |
...
| Элемент | Источник |
|---------|----------|
| Caption | `msg://` / `mainMsg://` из `com.company.hunttech.web.screens.openpositioncomment` |
