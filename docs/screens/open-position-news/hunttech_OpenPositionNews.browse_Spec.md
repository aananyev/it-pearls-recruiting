# OpenPositionNews Browse (`hunttech_OpenPositionNews.browse`)
...
Контроллер `hunttech_OpenPositionNews.browse`; навигация и дочерние формы — §3 «Иерархия и взаимосвязь форм».
...
| Параметр | Значение |
|----------|----------|
| **@UiController** | `hunttech_OpenPositionNews.browse` |
| **Java-класс** | `com.company.hunttech.web.screens.openpositionnews.OpenPositionNewsBrowse ` |
...
| **Меню** | `web-menu.xml` → `screen="hunttech_OpenPositionNews.browse"` (если есть пункт) |
...
```
select e from hunttech_OpenPositionNews e
```
...
| Парный экран | `hunttech_OpenPositionNews.edit` | create / edit action |
