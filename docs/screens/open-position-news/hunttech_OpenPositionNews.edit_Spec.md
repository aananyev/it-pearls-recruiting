# OpenPositionNews Edit (`hunttech_OpenPositionNews.edit`)

> Сущность: [OpenPositionNews.md](../../entities/open-position-news/OpenPositionNews.md)
...
Контроллер `hunttech_OpenPositionNews.edit`; навигация и дочерние формы — §3 «Иерархия и взаимосвязь форм».
...
| Параметр | Значение |
|----------|----------|
| **@UiController** | `hunttech_OpenPositionNews.edit` |
| **Java-класс** | `com.company.hunttech.web.screens.openpositionnews.OpenPositionNewsEdit ` |
| **XML-дескриптор** | `open-position-news-edit.xml` |
| **messagesPack** | `com.company.hunttech.web.screens.openpositionnews` |
| **Базовый класс** | `StandardEditor` |
| **Lookup-компонент** | `` |
| **EditedEntityContainer** | `openPositionNewsDc` |
| **focusComponent** | `newsVBox` |
| **Меню** | `web-menu.xml` → `screen="hunttech_OpenPositionNews.edit"` (если есть пункт) |
...
```
select e from hunttech_OpenPosition e order by e.openClose, e.vacansyName
```
...
| Парный экран | `hunttech_OpenPositionNews.browse` | create / edit action |
...
| Элемент | Источник |
|---------|----------|
| Caption | `msg://` / `mainMsg://` из `com.company.hunttech.web.screens.openpositionnews` |
