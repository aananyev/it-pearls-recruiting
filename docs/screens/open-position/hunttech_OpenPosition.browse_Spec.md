# OpenPosition Browse (`hunttech_OpenPosition.browse`)

> Списочный экран вакансий HRM HuntTech.
> Сущность: [OpenPosition.md](../../entities/open-position/OpenPosition.md) · [OpenPosition_Spec.md](../../entities/open-position/OpenPosition_Spec.md)
...
Меню → `hunttech_OpenPosition.browse` (icon `COMPASS`, lookup 1000×800). Дочерние: Edit, detail-фрагмент, комментарии, `Suggestjobcandidate`, `JobCandidateSimpleBrowse`, групповая подписка `RecrutiesTasksGroupSubscribeBrowse`, quick view описания.
...
| **Java-класс** | `com.company.hunttech.web.screens.openposition.OpenPositionBrowse` |
...
| **Меню** | `web-menu.xml` → `screen="hunttech_OpenPosition.browse"`, icon `COMPASS` |
| **Режим диалога** | 1000×800 |
| **Загрузка данных** | `@LoadDataBeforeShow` |

### Назначение

Основной browse вакансий: иерархия (`parentOpenPosition`), срочные вакансии, фильтры подписки/приоритета/удалёнки, rich-колонки (логотипы, рейтинг, статистика CV), details с фрагментом `hunttech_OpenPositionDetailScreenFragment` и Skillsbar.
...
```mermaid
flowchart TD
    Menu[web-menu.xml] --> Browse[hunttech_OpenPosition.browse]
    Browse --> Edit[hunttech_OpenPosition.edit]
    Browse --> Fragment[hunttech_OpenPositionDetailScreenFragment]
    Browse --> CommentEdit[OpenPositionComment.edit]
    Browse --> CommentsView[OpenPositionCommentsView]
    Browse --> Suggest[hunttech_SuggestJobCandidate]
    Browse --> QuickView[hunttech_QuickViewOpenPositionDescription]
    Browse --> GroupSub[RecrutiesTasksGroupSubscribeBrowse]
    Browse --> JCSimple[hunttech_JobCandidateSimple.browse]
```

| Связь | Экран | Открытие |
|-------|-------|----------|
| Edit | `hunttech_OpenPosition.edit` | create/edit actions |
| Details | `hunttech_OpenPositionDetailScreenFragment` | `detailsGenerator` |
| Комментарии | `hunttech_OpenPositionComment.edit`, `hunttech_OpenPositionCommentsView` | `setRatingButton` popup |
