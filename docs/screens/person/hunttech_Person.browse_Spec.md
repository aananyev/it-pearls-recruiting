# Person Browse (`hunttech_Person.browse`)
...
Контроллер `hunttech_Person.browse`; навигация и дочерние формы — §3 «Иерархия и взаимосвязь форм».
...
| Параметр | Значение |
|----------|----------|
| **@UiController** | `hunttech_Person.browse` |
| **Java-класс** | `com.company.hunttech.web.screens.person.PersonBrowse ` |
...
| **Меню** | `web-menu.xml` → `screen="hunttech_Person.browse"` (если есть пункт) |
...
```
select e from hunttech_Person e order by e.secondName, e.firstName
```
...
| Парный экран | `hunttech_Person.edit` | create / edit action |
