# Position Browse (`hunttech_Position.browse`)

> Сущность: [Position.md](../../entities/position/Position.md)
...
Контроллер `hunttech_Position.browse`; навигация и дочерние формы — §3 «Иерархия и взаимосвязь форм».
...
| Параметр | Значение |
|----------|----------|
| **@UiController** | `hunttech_Position.browse` |
| **Java-класс** | `com.company.hunttech.web.screens.position.PositionBrowse ` |
| **XML-дескриптор** | `position-browse.xml` |
| **messagesPack** | `com.company.hunttech.web.screens.position` |
| **Базовый класс** | `StandardLookup` |
| **Lookup-компонент** | `positionsTable` |
| **EditedEntityContainer** | `` |
| **focusComponent** | `positionsTable` |
| **Меню** | `web-menu.xml` → `screen="hunttech_Position.browse"` (если есть пункт) |
...
```
select e from hunttech_Position e order by e.positionRuName
```
...
| Парный экран | `hunttech_Position.edit` | create / edit action |
...
| Элемент | Источник |
|---------|----------|
| Caption | `msg://` / `mainMsg://` из `com.company.hunttech.web.screens.position` |
