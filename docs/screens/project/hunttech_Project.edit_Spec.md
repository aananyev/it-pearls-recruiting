...
### 4.4 Обработка загружаемого логотипа (автоматическая, AI-first)
...
1. **AI-этап** (если `hunttech.projectLogo.ai.enabled=true`): AI-функция `PROJECT_LOGO_IMAGE_GENERATE` (capability `IMAGE_GENERATION`, провайдер OpenAI, модель `gpt-image-2`) удаляет фон нейросетью. Промпт/модель/подключение настраивает администратор в «Управление AI»; конфигурация сервиса — `HunttechProjectLogoConfig`.
2. **Классический финал** (всегда): конвертация в PNG; если сторона больше 300px — пропорциональное уменьшение; flood-fill удаление белого фона от краёв (порог 235, страховка после AI); вписывание в круг — квадратный канвас со стороной, равной диагонали логотипа (с запасом 5%), при отображении в круглом аватаре `ovaFallbackImage` углы не обрезаются.
3. **AI недоступен** (функция не активна, нет credentials, таймаут/ошибка) → лог `warn`, автоматический классический конвейер — загрузка никогда не прерывается.

Обработанный файл записывается в файловое хранилище, дескриптор получает расширение `png`. При ошибке обработки (например, загружен не-растровый файл) сохраняется исходный файл. Сервис: `ProjectLogoImageProcessingService` (web-контекст получает его через CUBA service proxy — запись в `WebRemoteProxyBeanCreator`).
...
| Дата | Изменение |
|------|-----------|
| 2026-08-13 | §4.4: логотип переведён на AI-first (функция `PROJECT_LOGO_IMAGE_GENERATE`, capability IMAGE_GENERATION, OpenAI `gpt-image-2`) с классическим fallback; исправлена web↔core интеграция сервиса (запись в `WebRemoteProxyBeanCreator` устранила `NoSuchBeanDefinitionException` при загрузке логотипа) |
