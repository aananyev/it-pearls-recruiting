# SkillTree Browse (`hunttech_SkillTree.browse`)
...
Контроллер `hunttech_SkillTree.browse`; навигация и дочерние формы — §3 «Иерархия и взаимосвязь форм».
...
| Параметр | Значение |
|----------|----------|
| **@UiController** | `hunttech_SkillTree.browse` |
| **Java-класс** | `com.company.hunttech.web.screens.skilltree.SkillTreeBrowse ` |
| **XML-дескриптор** | `skill-tree-browse.xml` |
| **messagesPack** | `com.company.hunttech.web.screens.skilltree` |
| **Базовый класс** | `StandardLookup` |
| **Lookup-компонент** | `skillTreesTable` |
| **EditedEntityContainer** | `` |
| **focusComponent** | `skillTreesTable` |
| **Меню** | `web-menu.xml` → `screen="hunttech_SkillTree.browse"` (если есть пункт) |
...
```
select e from hunttech_SkillTree e
```
...
| Парный экран | `hunttech_SkillTree.edit` | create / edit action |
...
| Элемент | Источник |
|---------|----------|
| Caption | `msg://` / `mainMsg://` из `com.company.hunttech.web.screens.skilltree` |
