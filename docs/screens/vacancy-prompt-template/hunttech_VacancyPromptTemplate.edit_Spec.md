...
Открывается из `hunttech_VacancyPromptTemplate.browse` (create/edit) или программно.
...
Подсказки полей формулированы на понятном для пользователей языке (без технического жаргона). Локализация: `messages.properties` / `messages_ru.properties` пакета экрана (`com.company.hunttech.web.screens.vacancyprompttemplate`).
