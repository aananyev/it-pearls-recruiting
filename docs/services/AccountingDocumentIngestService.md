...
`AccountingDocumentIngestService` принимает файл, уже скачанный Telegram-ботом, сохраняет его во входящую папку Yandex.Disk и создает запись реестра `AccountingDocument` в HRM HuntTech.
...
Пользовательского экрана нет. Вход в процесс — Telegram-бот HRM HuntTech.

Технический вызов:

| Источник | Вызов |
| -------- | ----- |
| `Bot.processNonCommandUpdate` | получает Telegram `photo` или `document`, скачивает файл по `fileId`, вызывает `AccountingDocumentIngestService.ingestTelegramFile(...)` |

## Behavior Summary

| Действие | Условие | Результат |
| -------- | ------- | --------- |
| Пользователь отправил фото | Telegram message содержит `photo` | Бот скачивает максимальный размер фото и передает в сервис. |
