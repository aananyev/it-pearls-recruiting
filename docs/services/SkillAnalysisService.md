...
| system_prompt | Русский: извлечение навыков из резюме/вакансии по уровню анализа; каждый навык — РОВНО один уровень (MAIN > SECONDARY > TERTIARY), дубли между уровнями запрещены; названия из текста, запрет выдумывания, опыт в годах — РОВНО ОДИН навык (общий стаж кандидата, при диапазоне — максимум, менее года — «1 год»), строго JSON-массив |
...
> **Уровни непересекающиеся (контракт с 260816-5).** Промпт требует относить каждый
> навык РОВНО к одному уровню (`MAIN` > `SECONDARY` > `TERTIARY`) и запрещает дубли
> между уровнями. Причина контракта: для резюме модель возвращала для `SECONDARY`
> те же навыки, что и для `MAIN`, а кросс-уровневая дедупликация в `CandidateCVEdit`
> молча отбрасывала дубли — в сайдбаре «Основные навыки» блок «Желательные» оставался
> пустым, вторичные навыки не сохранялись в `HUNTTECH_CANDIDATE_SKILL`. Для резюме
> `MAIN` — навыки, на которых строится профессия (языки, основные технологии,
> фреймворки); `SECONDARY` — заметно упомянутые, но не ключевые (разделы
> «Дополнительно»/«Прочее», менее значимые инструменты); `TERTIARY` — редко
> упомянутые. Если навык можно отнести к нескольким уровням — более высокий
> выигрывает (это же правило дублирует дедупликация на экране: навык сохраняется
> с приоритетом своего высшего уровня).
...
| `modules/core/test/com/company/hunttech/core/SkillAnalysisAiFunctionSeedContractTest.java` | seed-контракт: INSERT-only/идемпотентность, TEXT_GENERATION, русские промпты с `${sourceText}`/`${skillLevel}` и требованием JSON-массива, include в master, 4 метода интерфейса, proxy в web-spring.xml, регистрация UPDATE-миграций 260816-3 (единый навык опыта) и 260816-5 (дизъюнктные уровни навыков, защита админской настройки) |
