...
Sidebar buttons presentation-only; footer использует `windowCommitAndClose`/`windowClose`. Кнопки footer обёрнуты в паттерн эталона IteractionListEdit: `hbox editFooterActions` (`edit-footer-actions`) → `expand=editActionsSpacer` (vbox 1px) → `hbox editActionsGroup` (AUTO, MIDDLE_RIGHT) — кнопки прижаты в правый нижний угол. OK получает `ai-function-configuration-primary-action` (белый текст на primary), Отмена — `ai-function-configuration-secondary-action` (прозрачный фон).
