...
- загрузка DOC → HWPF-экстрактор Apache POI (`WordExtractor`, poi-scratchpad) извлекает текст → текст записывается в `candidateCVRichTextArea`;
- загрузка RTF → встроенный в JDK `RTFEditorKit` извлекает plain text без разметки → текст записывается в `candidateCVRichTextArea`;
...
- lazy CV, PDF/DOC/DOCX/RTF parsing, recognition, text conversion и original view;
...
| Дата | Изменение |
|---|---|
| 2026-08-16 | Тематические пиктограммы и распознавание форматов резюме: вкладкам tabSheet добавлены иконки FontAwesome (Кандидат — `USER`, Резюме — `FILE_TEXT_O`, Письмо — `SEND`, Навыки — `SITEMAP`, Файлы — `FOLDER_O`) — единый визуальный язык с пунктами label-навигации sidebar (SCSS: `.candidate-cv-tabs .v-caption .v-icon`); кнопки «Загрузить»/«Очистить» файловых полей карточек «Оригинальное резюме» и «Резюме HRM HuntTech» (FileUploadField → `.c-fileupload`) выровнены по центру по вертикали и горизонтали (`flex` на `.v-button-wrap` + `text-align: center`) и получили тематические пиктограммы `fa-upload` (`\f093`) и `fa-trash-o` (`\f1f8`) через `:before`; распознавание загруженного резюме расширено с PDF до DOC/DOCX/RTF: `.doc` извлекается HWPF-экстрактором Apache POI (`WordExtractor`, добавлена зависимость `poi-scratchpad:4.1.2` в `build.gradle`, согласована с транзитивным POI 4.1.2 от HhJavaApi), `.rtf` — встроенным в JDK `RTFEditorKit` (plain text без разметки), `.docx` — существующим `XWPFWordExtractor`; текст любого формата попадает в lazy-managed `candidateCVRichTextArea` вкладки «Резюме» (общий код конца обработчика); заглушка «Функция загрузки .doc пока не реализована» удалена; SCSS синхронизирован во все 7 тем (4 канонические идентичны, Antigravity-темы сохранили свои nav-отличия); контрактный тест `CandidateCVEditVisualContractTest` обновлён (иконки вкладок, SCSS-блоки, Java-контракт doc/rtf) |
