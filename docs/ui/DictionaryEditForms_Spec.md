...
| Дата | Изменение |
|---|---|
| 2026-08-13 | Серия расширена до 10 форм: `SpecialisationEdit`, `OwnershupEdit`, `PositionEdit` переведены на общий контракт (sidebar 270px + ovalImage-иллюстрация + label-навигация + edit-card + footer); у шести прежних форм (`FileType`, `Grade`, `Currency`, `OutstaffingRates`, `EmployeeWorkStatus`, `SignIcons`) статичный `ovaFallbackImage` с `fallbackThemePath=icons/hunttech-logo.png` заменён на `ovalImage` с тематическим theme-ресурсом `icons/dictionaries/{form}.png` (эталон JobCandidateEdit); у `SpecialisationEdit` sidebar-навигация переключает вкладки TabSheet (`initTabNavigation`, эталон IteractionEdit); `PositionEdit` получил карточки «Наименование»/«Описание» и `focusMainSection`/`focusDescriptionSection`; в `dictionary-edit-forms.scss` (7 тем) namespace расширен тремя новыми формами; добавлены caption-ключи ownershup (`msgShortType`/`msgLongType`); тесты `DictionaryEditFormIllustrationContractTest` и `DictionaryEditSidebarRedesignContractTest`. |
