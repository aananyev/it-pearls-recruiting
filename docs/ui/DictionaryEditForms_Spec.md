...
> Экраны: `FileTypeEdit`, `SocialNetworkTypeEdit`, `GradeEdit`, `CurrencyEdit`, `OutstaffingRatesEdit`, `EmployeeWorkStatusEdit`, `SignIconsEdit`
> Дата: 2026-08-13

## Назначение и бизнес-смысл (What & Why)

Серия из семи Edit-форм обслуживает базовые справочники HRM HuntTech: типы файлов (категоризация документов и вложений), типы социальных сетей (название, URL и логотип контактов кандидатов), грейды (уровни квалификации и вилки оплаты), валюты (зарплатные вилки и финансовые расчёты), рейты по аутстафу (тарифная шкала ступеней с маржинальностью на уровне БД), статусы сотрудника (принадлежность к штату) и иконки признаков (font-маркеры типов признаков в профилях). Единая композиция форм снижает когнитивную нагрузку рекрутера при ведении справочников и соответствует общему контракту Edit-экранов HRM HuntTech.

## UI Context & Navigation

Все формы открываются из соответствующих browse-справочников (create/edit). Каждая использует обязательную двухпанельную композицию: постоянная sidebar 270px слева и рабочая область справа. Пять форм (`FileType`, `SocialNetworkType`, `Grade`, `Currency`, `EmployeeWorkStatus`) — одна карточка «Основные данные» и один пункт навигации. `OutstaffingRatesEdit` — две карточки («Ставки», «Комментарий») и два пункта навигации; `SignIconsEdit` — две карточки («Иконка», «Описание») и два пункта навигации. Единственная форма с загрузкой изображения — `SocialNetworkTypeEdit` (upload логотипа в sidebar); остальные шесть показывают штатную заглушку-логотип HRM HuntTech в круглом `OvaFallbackImage` 176×176.

## Behavior Summary

| Действие | Условие | Результат |
|---|---|---|
| открыть форму | editor загружает штатный контейнер | отображаются прежние данные в новой композиции; sidebar показывает живой title из свойства контейнера |
| нажать label-навигацию | выбран раздел текущей формы | фокус переводится к первому полю секции; активный пункт получает `label-nav-item-active`; entity и loaders не меняются |
...
- root namespaces: `file-type-editor`, `social-network-type-editor`, `grade-editor`, `currency-editor`, `outstaffing-rates-editor`, `employee-work-status-editor`, `sign-icons-editor`;
...
- штатная заглушка: `ovaFallbackImage` 176×176 (`ovalWidth/ovalHeight=176px`, `scaleMode=SCALE_DOWN`, `fallbackThemePath=icons/hunttech-logo.png`, stylename `dictionary-logo-image`) — у всех семи форм; у соцсети дополнительно `dataContainer=socialNetworkTypeDc property=logo` и upload `snLogoFileUpload`;
...
- контент: `edit-card` (карточки разделов, `showAsPanel="true"` — рендер Vaadin Panel, чтобы заголовок `.v-panel-caption` получал контрактный стиль);
