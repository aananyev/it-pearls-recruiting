...
Стили подключены через локальный mixin `user-ai-profile` и ограничены корнем `.user-ai-profile-editor`. Глобальные `.v-*` правила не добавлялись. При ширине окна 1200 px горизонтальная прокрутка не требуется; содержимое справа прокручивается вертикально. Вкладки «Интерфейс», «Почта» и AI сохраняют действующие component ID и функциональные контракты.
...
Обязательны `git diff --check`, компиляция web, `ScreenViewIntegrityTest` (8/8), Data View Integrity, сборка SCSS, `clean assemble`, локальный deploy, HTTP 200 и smoke вкладок «Обо мне», «Интерфейс», «Почта» и AI в теме Halo. Production-проверки и любые действия на PROD запрещены.
