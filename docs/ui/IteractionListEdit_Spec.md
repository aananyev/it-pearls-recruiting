...
зелёная поверхность `#008000` с принудительной белой подписью `#ffffff !important`
(класс `.v-button-caption` с `font-weight: 600; font-size: 13px`), скругление `10px`
и светлая рамка `rgba(81, 255, 0, 0.55)`; hover затемняет фон до `#006400`. Остальное
оформление (высота 64px, равные ячейки 5 × 20%, до трёх строк caption)
сохраняется без изменений. В disabled-позиции текст `Нет данных` имеет контрастный цвет,
а локальный стиль `.iteraction-list-popular-button` не меняет обработчик, список или данные.
...
| Дата | Изменение |
|---|---|
| 2026-09-22 | Исправлена инициализация календарного блока в `IteractionListEdit`: повторяющиеся calendar path удаляются перед построением `OptionsMap`, блок больше не скрывается при ошибке загрузки и остаётся видимым с отключёнными контролами и подсказкой; при смене типа с календарного на обычный выполняется отключение прежнего события. Добавлены регрессионные проверки. |
| 2026-09-16 | Завершена интеграция с Яндекс Календарём (Яндекс 360 CalDAV) и устранена проблема невидимости текста на 5 зелёных кнопках «Частые взаимодействия»: в SCSS всех 7 тем подавлен псевдоэлемент `:before` (`display: none !important; height: 0; width: 0;`), внедрено центрирование `.v-button { display: inline-flex !important; align-items: center; justify-content: center; }` и `.v-button-wrap { display: flex !important; width: 100% !important; height: 100% !important; z-index: 2; position: relative; }` с гарантированным контрастным белым текстом `.v-button-caption { color: #ffffff !important; z-index: 3; position: relative; text-shadow: 0 1px 2px rgba(0,0,0,0.6); }` и выносом media-запросов на верхний уровень; в контроллере `IteractionListEdit` обеспечена синхронизация полей `addToCalendar` и `calendarId` в DataContext/DB перед коммитом (`onBeforeCommitChanges`) и постоянная доступность `calendarBox` при активной дате встречи (`addDate.isVisible()`) с подсказкой `msgNoCalendarsAvailable` при отсутствии настроенных календарей. Проверено в живом браузере через CDP под пользователем `alan` и покрыто тестами `IteractionListMostPopularInteractionTest` и `CorporateYandexCalendarContractTest`. |
| 2026-09-09 | По контракту разработки форм Edit (Two-Pane Split View): ширина левого сайдбара стандартизирована до 312px во всех 7 темах SCSS (`.iteraction-list-sidebar` и `.v-slot-iteraction-list-sidebar`); на быстрых зеленых кнопках «Частые взаимодействия» зафиксирован принудительный белый цвет подписи `#ffffff !important; font-weight: 600; font-size: 13px` для `.v-button-caption` (устранено наследование `color: inherit`, приводившее к невидимости текста в светлых темах) с гарантированной динамической подписью «N. Название» и tooltip `description`, пустые слоты снабжены контрастным текстом «Нет данных». 100% бизнес-логики и bindings сохранены. |
