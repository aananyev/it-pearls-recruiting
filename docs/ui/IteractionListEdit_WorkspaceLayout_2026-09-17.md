...
| 2026-09-18 | Устранена невидимость кнопок «ОК» и «Отмена»: правая рабочая область `.iteraction-list-workspace` оформлена как адаптивная flex-колонка с зафиксированным footer действий (`editActions`), сброшен инлайновый `top` слотов, устранен double-padding; сняты избыточные `height="100%"` и цепочка `expand` с карточек внутри `scrollBox`. Реализована адаптивная верстка с автоматическим включением вертикального и горизонтального скроллинга при малых разрешениях экрана (`@media (max-height: 600px)` и `@media (max-width: 768px)`), исключая выход кнопок за границы экрана; sidebar 312px и зеленые кнопки сохранены без изменений. |
