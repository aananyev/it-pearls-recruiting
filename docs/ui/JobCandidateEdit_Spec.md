...
| `tabSheetSocialNetworks` | `tabSheet` | подключение постзагрузочной гидратации логотипов на `tabResume` |
...
Stage 2 не меняет CRUD резюме, распознавание контактов, копирование CV, проверку навыков, загрузку файлов, связи CV с вакансией и реализацию генератора `projectLogoColumn`.
...
- сохранение после открытия вкладки не создаёт дубликаты и не коммитит изменения `Project` или `OpenPosition`;
- Stage 1 по взаимодействиям остаётся зелёным;
- в логах отсутствуют `Cannot get unfetched attribute [candidateCv]`, `Cannot get unfetched attribute [projectLogo]`, detached-ошибки, `NullPointerException` и `OutOfMemoryError`.
