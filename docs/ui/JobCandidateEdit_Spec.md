...
### Этап 1 клиентской оптимизации SCSS

Базовые измерения одного тёплого открытия формы в теме Hover:

| Метрика | До оптимизации |
|---|---:|
| `ScriptDuration` | 1 265 ms |
| `RecalcStyleDuration` | 305 ms |
| `RecalcStyleCount` | 3 139 |
| `LayoutDuration` | 76 ms |
| `LayoutCount` | 44 |
...
- правила sidebar сведены к `job-candidate-sidebar-card` вместо повторяющихся цепочек для двух карточек;
- для `job-candidate-form-grid` удалены селекторы `td:nth-child(2)` и глубокие цепочки до внутренних полей;
- slot-компоненты основной формы растягиваются одним локальным правилом;
- повторяющиеся комбинации `width/min-width/max-width` сокращены там, где геометрия задаётся XML или flex layout;
- критические `!important` сохранены только для inline-размеров Vaadin и конфликтующих правил тем.

Целевые значения после A/B-профилирования:

| Метрика | Цель этапа 1 |
|---|---:|
| `ScriptDuration` | не более 1 000 ms |
| `RecalcStyleDuration` | не более 150 ms |
| `RecalcStyleCount` | не более 1 500 |
...
- повторное клиентское профилирование с расчётом дельт одного открытия.
