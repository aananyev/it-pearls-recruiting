...
- Открытие кандидата → перед `InitEvent` из runtime-view последовательно исключаются `iteractionList` и `candidateCv` → основная карточка открывается без предварительной материализации взаимодействий и резюме.
...
- Повторное открытие вкладки → проверяются флаги инициализации → повторная полная загрузка резюме и дублирование строк не выполняются.
...
| `tabSheetSocialNetworks` | `tabSheet` | подключение постзагрузочной гидратации логотипов на `tabResume`, `tabContactInfo` и `tabSocialNetworks` |
...
- `socialNetwork` на Stage 2 остаётся в стартовом view для изолированного измерения эффекта;
...
- после CRUD социальных сетей флаг гидратации сбрасывается для нового набора справочников;
...
- сохранение кандидата без открытия вкладок не должно удалять или обнулять существующие взаимодействия и резюме.
...
Stage 2 не меняет CRUD резюме, распознавание контактов, копирование CV, проверку навыков, загрузку файлов, связи CV с вакансией, CRUD социальных сетей и реализации генераторов `projectLogoColumn` и `socialNetworkLogoColumn`.
...
Stage 2 не содержит изменений SCSS.
...
Ручная проверка Stage 2:

- HTTP 200 для `/hrm`;
- открыть существующего кандидата с CV и проверить «Резюме: ДА»;
- открыть существующего кандидата без CV и проверить «Резюме: НЕТ»;
- открыть нового кандидата и убедиться в отсутствии unfetched/detached ошибок;
- до открытия `tabResume` подтвердить отсутствие материализации полной коллекции `candidateCv`;
- допустим отдельный скалярный `COUNT` для индикатора;
- первое открытие `tabResume` загружает все штатные строки;
- проекты связанных вакансий догружаются одним запросом, а логотипы отображаются без `Cannot get unfetched attribute [projectLogo]`;
- CV без вакансии и вакансия без проекта отображаются без исключений;
- несколько CV одного проекта не вызывают повторную загрузку проекта в пределах одной гидратации;
- повторное переключение на `tabResume` не создаёт дубликаты и не выполняет лишнюю полную загрузку;
- создание, редактирование, удаление или отмена CV сохраняют прежнее поведение;
- после CRUD резюме логотип нового проекта отображается без исключения;
- открыть вкладки `tabContactInfo` и `tabSocialNetworks` для кандидата с несколькими соцсетями;
- логотипы типов соцсетей отображаются без `Cannot get unfetched attribute [logo]`;
- несколько строк одного типа соцсети не создают повторные справочники и дубликаты;
- строка без выбранного типа соцсети отображается без исключения;
- после добавления или редактирования соцсети логотип отображается без повторного открытия карточки;
- сохранение кандидата после открытия вкладок не коммитит изменения `Project`, `OpenPosition` или `SocialNetworkType`;
- сохранение кандидата без открытия `tabResume` не удаляет существующие CV;
- сохранение после открытия вкладки не создаёт дубликаты;
- Stage 1 по взаимодействиям остаётся зелёным;
- в логах отсутствуют `Cannot get unfetched attribute [candidateCv]`, `Cannot get unfetched attribute [projectLogo]`, `Cannot get unfetched attribute [logo]`, detached-ошибки, `NullPointerException` и `OutOfMemoryError`.
