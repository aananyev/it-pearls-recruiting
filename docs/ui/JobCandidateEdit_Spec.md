...
XML уже содержит контейнеры `job-candidate-accordion-header` и `job-candidate-accordion-content`. В предыдущей версии SCSS заголовок ошибочно скрывался правилом `display: none`, поэтому accordion-слой не отображался.

Исправленный визуальный контракт:

- заголовок раздела отображается над содержимым;
- слева выводится маркер раскрытого раздела;
- заголовок и содержимое образуют единую карточку;
- заголовок не удаляет и не подменяет штатную навигацию `TabSheet`;
- поведение полей и таблиц не меняется.
