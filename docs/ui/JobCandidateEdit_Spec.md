...
| `fileImageFaceUpload` | умная обработка фотографии (`WebProjectLogoFileUploadField`: PNG, ресайз 300px, удаление белого фона, круг) → IMMEDIATE в `fileImageFace` → OvaFallbackImage перечитывает контейнер; «Очистить» → сброс значения; повторная загрузка в том же экране обрабатывается заново |
