...
- всё внутреннее содержимое сайдбара помещено в полновысотный `<scrollBox id="candidateSidebarScroll" width="100%" height="100%" orientation="vertical">`, предотвращающий обрезание контента на экранах с небольшой высотой;
...
- фотография отображается единым `OvaFallbackImage` размером 120×120 и не искажается;
- при отсутствии фотографии или бинарного файла показывается `icons/no-programmer.jpeg`;
- загрузка фото IMMEDIATE проходит умный конвейер (общий с ProjectEdit/CompanyEdit — `WebProjectLogoFileUploadField` + `ProjectLogoImageProcessingService`, конфиг `hunttech.projectLogo.*`): PNG, ресайз до 300×300, удаление фона ТОЛЬКО нейросетью rembg (u2net; AI-функция и классический flood-fill для фото людей не применяются, пропорции сохраняются, в круг не вписывается) — затем пишет файл в `fileImageFace`; повторная загрузка без закрытия экрана обрабатывается заново; если фон действительно удалён нейросетью — показывается исчезающая TRAY-нотификация «Фотография обработана с помощью AI» (стандартный механизм CUBA, `Notifications`, правый нижний угол); если rembg недоступен, фото только конвертируется/ресайзится и нотификация НЕ показывается;
- ФИО в формате `Фамилия Имя`, должность и город проживания видны непосредственно под фотографией с центрированием и классами `candidate-sidebar-fullname bold`, `candidate-sidebar-position bold`, `candidate-sidebar-city bold`;
...
- `candidateNavigation` использует общие классы `label-navigation`, `label-nav-title`,
  `label-nav-item`, `label-nav-item-active`; заголовок «Разделы формы» дополнительно несёт
  локальный класс `job-candidate-section-title` — полоса-заголовок с двумя горизонтальными
  inset-линиями (контракт §4.1); локальный `job-candidate-nav-item` оставлен
...
| `fileImageFaceUpload` | умная обработка фотографии (`WebProjectLogoFileUploadField`: PNG, ресайз 300px, удаление фона нейросетью rembg — AI-функция/классика/круг для фото не применяются) → IMMEDIATE в `fileImageFace` → OvaFallbackImage перечитывает контейнер; «Очистить» → сброс значения; повторная загрузка в том же экране обрабатывается заново; при реальном нейросетевом удалении фона — исчезающая TRAY-нотификация «Фотография обработана с помощью AI» |
...
| 2026-09-24 | **Устранение выхода комментариев и фотографии пользователя за правую границу Box (BL-2026-023, presentation-only)**: устранена первопричина выталкивания аватарки и тела комментария. В `JobCandidateEdit.java` (`buildCommentComponent`) `outerBox` переведен с `setWidthFull()` на `setWidthAuto()`, устранен конфликт с `flex: 1 1 0` и жесткий расчет ширины Vaadin; `innerBox.setSpacing(false)` исключил вставку фантомных `.v-spacing`. Во всех 7 темах SCSS: слот фото `.v-slot-circle-50px` и изображение `.circle-50px` получили строгий размер `flex: 0 0 50px !important`, слот тела `.v-slot-job-candidate-comment-body` получил `flex: 1 1 0 !important; max-width: 100%`, в ленте добавлен `padding-right: 6px` против наложения полосы прокрутки. Все элементы комментария строго остаются в границах Box на любых экранах и при любом количестве сообщений. |
| 2026-09-23 | **Исправление компоновки блока комментариев (BL-2026-023, presentation-only)**: в `JobCandidateEdit.java` (`buildCommentComponent`) `outerBox`, `text`, `name`, `vacancy` переведены на `widthFull()`, `date` и `replyButton` объединены в компактный горизонтальный `footerBox` (`.job-candidate-comment-footer`). Во все 7 SCSS-тем добавлены правила `.job-candidate-comment-footer` и `.job-candidate-comment-reply` с `box-sizing: border-box`, исключающие вылезание элементов и раздувание пузыря при длинном тексте или узком viewport. |
