...
- пункты `label-nav-item` используют ритм эталона `IteractionListEdit`: высота 38px,
  вертикальное центрирование текста и маркера, шрифт 13px;
...
| Дата | Изменение |
|---|---|
| 2026-08-03 | Левая часть JobCandidateEdit доведена до эталона `IteractionListEdit`: label-навигация восстановлена до 38px/13px, tab captions больше не обрезаются ellipsis и доступны полностью через локальный `overflow-x`, `Загрузить`/`Очистить` под фото стали одинаковыми 96×36px, а «Сохранить и закрыть»/«Отмена» сгруппированы внизу справа. Изменены только локальные SCSS семи тем и визуальные контрактные проверки. |
