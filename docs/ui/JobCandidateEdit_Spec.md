...
- Повторное открытие вкладки → проверяется флаг инициализации → повторная полная загрузка и дублирование строк не выполняются.
...
Stage 2 не меняет CRUD резюме, распознавание контактов, копирование CV, проверку навыков, загрузку файлов и связи CV с вакансией.
...
- в логах отсутствуют `Cannot get unfetched attribute [candidateCv]`, `IllegalStateException`, `NullPointerException` и `OutOfMemoryError`.
