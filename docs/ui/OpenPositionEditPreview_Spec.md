...
- блок «Проект, Компания, Тип должности» → `projectLocationAccordion` получает локальный namespace, строка типа должности оставляет должность и удалённую работу на первой линии, комментарий переносится ниже, а проект и компания занимают полную ширину своих строк;
- поля project-блока → локальный visual layer сбрасывает legacy-смещения слотов и приводит picker/text controls к `width: 100%` без изменения component ID, bindings и бизнес-значений;
...
- таблица договоров → использует локальный визуальный вариант 5: theme-aware header, фиксированный ритм строк, ellipsis для длинных значений, единый editor и компактную панель действий → dataGrid, columns и actions не меняются;
- вкладка «Описание вакансии» → четыре существующих `RichTextArea` получают единый вариант 5 с полноразмерной областью редактирования, отдельным toolbar и стабильной высотой → properties, editable и bindings не меняются;
...
| Дата | Изменение |
|------|-----------|
| 2026-08-03 | Вкладки «Трудовой договор» и «Описание вакансии» приведены к визуальному варианту 5: таблица договоров получила theme-aware header, ритм строк и editor-поверхность, а четыре `RichTextArea` — единый toolbar/content-контракт и полноразмерную область. Изменены только локальные stylename и SCSS во всех семи темах; бизнес-логика и bindings не менялись. |
| 2026-08-03 | Блок «Проект, Компания, Тип должности» приведён к устойчивой компоновке: должность и удалённая работа выровнены в первой строке, комментарий перенесён во вторую, поля проекта и компании растянуты на ширину рабочей области, а legacy-смещения Vaadin slot нейтрализованы локальными ролями. Изменены только presentation-классы и их контракт во всех семи темах; бизнес-логика не менялась. |
