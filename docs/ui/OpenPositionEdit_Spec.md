...
│   │   └── 5 пунктов borderless label-nav-item: «Вакансия» (active по умолчанию на вкладке «Вакансия»), «Параметры вакансии», «Количество персонала», «Команда / Вакансия», «Зарплатное предложение» (пункты «Идентификаторы» и «Настройки вакансии» удалены 2026-08-06); рабочие label-навигации с кликом — «Вакансия» (фокус в ID), «Параметры вакансии» (фокус в должность), «Зарплатное предложение» (фокус в мин. зарплату); остальные — presentation-only без invoke
...
- **Блок «Вакансия»** (`openPositionEditorIdentifiersCard`, сверху) — плоская карточка, все строки напрямую (обёртки `vacancyInputGroupBox`, `commandFieldHBox`, `projectTypeGroupBox`, `closingDateFieldsHBox`, `vacancyParamsPositionRow`, `vacancyParamsProjectRow`, `projectFieldsVBox` удалены), строки сверху вниз:
  - **Строка 1** `vacancyTopRow` (`row-half`): внутренний ID `vacansyIDTextField` (`width=AUTO`) + грейд `gradeLookupPickerField` (`width=AUTO`, options `gradeDc`) + приоритет `priorityField` (required, `width=AUTO`, класс `open-position-editor-priority-field`) + удалёнка `remoteWorkField` (`width=AUTO`, required) + дата закрытия `closingDateDateField` — компактные поля по содержимому в одну линию (удалёнка перед датой закрытия);
  - **Строка 2** `vacancyMainRow` (`row-half`): должность `positionTypeField` (ratio 3, required, options `positionTypesDc`) + проект `projectNameField` (ratio 4, required, options `projectNamesDc`) — свободное место делится 3:4;
  - **Строка 3** `vacancyCompanyRow` (`row-half`): компания `companyNameField` + департамент `companyDepartamentField` (оба required, options из соответствующих Dc) — в одну линию, половина/половина;
  - **Строка 4** `vacancyCityRow` (`row-half`) → `cityFieldsHBox` (expand на город): город `cityOpenPositionField` (required, options `citiesDc`) + кнопка `addCity` («Добавить», `invoke` сохранён) — отдельной строкой под компанией/департаментом;
  - **Строка 5** `remoteCommentRow`: комментарий к формату удалённой работы `remoteWorkCommentField` (на всю ширину, expand);
  - **Строка 6** `vacancyNameHBox` (`row-title`): название вакансии `vacansyNameField` (required, `expand="vacansyNameField"` на hbox, width 100%, без expandRatio) + кнопка `generateVacancyNameFieldButton` «Генерировать» (`width=AUTO`, без expandRatio, `invoke` сохранён) — растягивается строка ввода, кнопка остаётся по содержимому;
  - **Строка 7** `vacancyFlagsRow`: все три чекбокса скрыты (`visible="false"`): `signDraftCheckBox` «Черновик вакансии», `internalProjectCheckBox` «Эксклюзивная вакансия» (Java-управление видимостью отключено — `setInternalProject()` больше не вызывает `setVisible`) и служебный `openClosePositionCheckBox` (Java работает только со значением);
  - **Строка 8** `projectFilterCheckBoxesHBox`: фильтры списка проектов `onlyOpenProjectCheckBox`/`withOpenPositionCheckBox`;
  - **Строка 9** `priorityFieldsHBox`: комментарий к приоритету `commentPriority` (caption «Комментарий», на всю ширину).
...
| `vacansyIDTextField` / `gradeLookupPickerField` / `vacansyNameField` | textField / lookupPickerField | `vacansyID` / `grade` / `vacansyName` | карточка «Вакансия» (`openPositionEditorIdentifiersCard`, `vacancyTopRow` / `vacancyNameHBox`) | id, dataContainer, property, required |
...
| `positionTypeField` | lookupPickerField | `positionType` | карточка «Вакансия» `openPositionEditorIdentifiersCard` (`vacancyTopRow`, focusComponent) | id, dataContainer, property, options, required |
