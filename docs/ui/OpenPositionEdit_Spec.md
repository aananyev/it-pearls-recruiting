...
- **Блок «Наименование»** (`openPositionNameGroupBox`, самый верх вкладки) — одна строка `openPositionNameRow` (`row-title`, без expand): внутренний ID `vacansyIDTextField` + грейд `gradeLookupPickerField` (options `gradeDc`) + название вакансии `vacansyNameField` (required) + кнопка `generateVacancyNameFieldButton` «Генерировать» (`invoke="generateNameFieldButton"` сохранён); ID, Грейд и кнопка «Генерировать» — фиксированная одинаковая ширина **110px** (ровно как поле ID), Вакансия растягивается на свободное место (`width=100%` + `box.expandRatio=1`); поля перенесены сюда из карточки «Вакансия» (строка `vacancyNameHBox` удалена, ID и Грейд убраны из `vacancyTopRow`);
...
| `vacansyIDTextField` / `gradeLookupPickerField` / `vacansyNameField` | textField / lookupPickerField | `vacansyID` / `grade` / `vacansyName` | блок «Наименование» (`openPositionNameGroupBox`, строка `openPositionNameRow`: ID/Грейд/кнопка — 110px, Вакансия expandRatio=1) | id, dataContainer, property, required, options |
