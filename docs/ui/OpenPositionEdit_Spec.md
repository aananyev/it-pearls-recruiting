...
- **Блок «Вакансия»** (`openPositionEditorIdentifiersCard`, сверху) — элементы бывших вложенных блоков «Вакансия» (`vacancyInputGroupBox`) и «Настройки вакансии» (`commandFieldHBox`) вынесены из groupBox-обёрток напрямую в карточку (обёртки удалены, captions «Вакансия»/«Настройки вакансии» упразднены; порядок строк и bindings сохранены) — 3 строки:
  - **Строка 1** `vacancyNameHBox` (`row-title`): внутренний ID `vacansyIDTextField` (ограничен, ratio 1) + название `vacansyNameField` (растягивается, ratio 5, required) + грейд `gradeLookupPickerField` (`width=AUTO`, options `gradeDc`) + кнопка `generateVacancyNameFieldButton` «Генерировать» (`width=AUTO`, `invoke` сохранён) — все в ряд;
  ...
  - **Строки блока «Параметры вакансии»** (обёртка `projectTypeGroupBox` упразднена — элементы перенесены в карточку «Вакансия», caption `msgVacancyParams` больше не выводится), строки сверху вниз:
...
| `vacansyIDTextField` / `vacansyNameField` / `gradeLookupPickerField` | textField / lookupPickerField | `vacansyID` / `vacansyName` / `grade` | карточка «Вакансия» (`openPositionEditorIdentifiersCard`, строка `vacancyNameHBox`) | id, dataContainer, property, required |
...
| `positionTypeField` | lookupPickerField | `positionType` | карточка «Вакансия» `openPositionEditorIdentifiersCard` (`vacancyParamsPositionRow`, focusComponent) | id, dataContainer, property, options, required |
| `projectNameField` / `companyDepartamentField` / `companyNameField` / `cityOpenPositionField` | lookupPickerField | `projectName` / — / — / `cityPosition` | карточка «Вакансия» `openPositionEditorIdentifiersCard` (строки проекта/компании/города) | id, options, property, required, actions |
