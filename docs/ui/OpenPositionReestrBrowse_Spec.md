...
### 4.6 Индикация закрытых вакансий и глобальные нотификации (CUBA Global Events)

1. **Первая колонка таблицы («Приоритет», ширина 75px)**:
   - Для открытых вакансий (`openClose = false` или `null`) отображает центрированный SVG-бейдж приоритета (Критический, Высокий, Обычный, Низкий, Приостановлена, Черновик, На проверку).
   - Для закрытых вакансий (`openClose = true`) формирует составной индикатор: значок закрытой вакансии (красный бейдж 20px с замочком 🔒 `#ef4444`) рядом со значком приоритета, а также всплывающую подсказку «Вакансия закрыта».
   - Признак отображается автоматически при нажатии кнопки «Закрыть» в сайдбаре, а также при показе каталога всех вакансий («Все, включая архивные / закрытые»). При открытии вакансии признак закрытия немедленно скрывается.

2. **Глобальные нотификации пользователей (CUBA Platform Global Events)**:
   - Передача событий между браузерными сессиями пользователей реализована через модуль `com.haulmont.addon.globalevents:cubaglevt-global:0.6.1` и событие `UiNotificationEvent` (`implements GlobalUiEvent`).
   - При операциях:
     - открытие вакансии (в `OpenPositionReestrBrowse` кнопкой «Открыть» либо в `OpenPositionEdit`);
     - закрытие вакансии (в `OpenPositionReestrBrowse` кнопкой «Закрыть» либо в `OpenPositionEdit`);
     - заведение новой вакансии (в `OpenPositionEdit` либо через визард «Умная загрузка» `SmartOpenPositionUploadScreen`),
   - Всем пользователям системы рассылается всплывающее немодальное TRAY-сообщение (`Notifications.NotificationType.TRAY`), исчезающее через **5 секунд** (`withHideDelayMs(5000)`).
   - Текст оповещения содержит наименование вакансии и её ключевые характеристики: проект, город/локация, формат работы (удалённо/офис/гибрид), зарплатное предложение и имя инициатора.
