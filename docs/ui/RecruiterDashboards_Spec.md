...
### Оформление и перенос текста внутри карточки
Для всех текстовых элементов внутри карточки (`.recruiter-kanban-card`) обеспечен перенос длинного текста по словам:
- Для всех `Label`, `LinkButton` и контейнеров задан `width: 100%`;
- В стилях всех 7 тем применены правила `white-space: normal !important; word-break: break-word !important; overflow-wrap: break-word !important; word-wrap: break-word !important;`;
- Исключено обрезание и горизонтальное распирание карточки при длинных ФИО, названиях проектов, вакансий и статусов.

## История изменений

| Дата | Изменение |
|---|---|
| 2026-09-26 | Обеспечен перенос по словам (`word-break: break-word`, `white-space: normal`) для всех элементов `Label` и заголовков внутри карточки канбана во всех 7 темах. |
| 2026-09-26 | Устранена ошибка `Unfetched Attribute [projectLogo]` при открытии `IteractionListEdit` из канбана (загрузка полного `openPosition-iteraction-list-picker-view` и безопасное чтение полей). |
