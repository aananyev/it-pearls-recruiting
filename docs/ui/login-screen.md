...
| `logoWrap` | `vbox` | stylename=`c-login-logo-wrap`, align=`MIDDLE_CENTER`, width=`100%` | Обёртка логотипа: прозрачный фон, рамка `1px solid #555`, `border-radius: 5px` |
...
| `.c-login-logo-wrap` | Обёртка логотипа: flex, `align-items: center`, ширина 100%, `border: 1px solid #555`, `border-radius: 5px`, прозрачный фон |
...
`c-login-layout`, `c-login-wrapper`, `c-login-card`, `c-login-logo-wrap`, `c-login-logo`, `c-login-welcome-label`, `h1`, `c-login-field`, `inline-icon`, `large`, `c-login-remember-me-checkbox`, `c-login-locales-select`, `borderless`, `c-login-button`, `primary`, `huge`, `c-login-bottom-panel`, `c-login-powered-by-link`, `c-login-background`.
