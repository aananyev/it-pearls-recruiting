...
-- AI относит каждый навык РОВНО к одному уровню анализа (MAIN > SECONDARY > TERTIARY)
-- и НЕ дублирует навыки между уровнями. Причина: для резюме модель возвращала для
-- SECONDARY те же навыки, что и для MAIN («дополнительные навыки» трактовались слишком
-- широко), а кросс-уровневая дедупликация в CandidateCVEdit молча отбрасывала дубли —
-- в сайдбаре «Основные навыки» блок «Желательные» оставался пустым.
-- Административная настройка, изменённая администратором, не перезаписывается
-- (тот же контракт, что и 260814-3); для свежих сред — INSERT с новой версией промпта.
