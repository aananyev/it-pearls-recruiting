...
/**
 * Провайдер для Xiaomi MiMo (Moonshot AI).
 * <p>
 * MiMo — это LLM-модель от Xiaomi, работающая через API Moonshot AI.
 * API полностью совместим с форматом OpenAI (Chat Completions), что позволяет
 * использовать аналогичную логику запросов.
 * <p>
 * Endpoint: https://api.moonshot.cn/v1/chat/completions
 * Бесплатные кредиты: ~15¥ при регистрации (ок. 10M токенов)
 * <p>
...
public class MiMoProvider implements AIProvider {
    ...
    public String generateText(String prompt, String systemContext, String apiKey, String modelName,
        ...
        try {
            ...
            log.error("Xiaomi MiMo (Moonshot AI) API request failed: {}", e.getMessage(), e);
            throw new RuntimeException("Ошибка запроса к Xiaomi MiMo API: " + e.getMessage(), e);
        }
    }
