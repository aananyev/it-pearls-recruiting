    ...
    public void testBrowseLoadCity() {
        ...
        List<City> cities = dataManager.loadList(LoadContext.create(City.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_City e where e.id = :id")
    ...
    public void testSoftDeleteCity() {
        ...
        try (Transaction tx = persistence.createTransaction()) {
            City deleted = persistence.getEntityManager()
                    .createQuery("select e from hunttech_City e where e.id = :id", City.class)
                    .setParameter("id", id)
                    .getSingleResult();
            tx.commit();
        ...
        List<City> active = dataManager.loadList(LoadContext.create(City.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_City e where e.id = :id and e.deleteTs is null")
                        .setParameter("id", id))
                .setView(View.LOCAL));
