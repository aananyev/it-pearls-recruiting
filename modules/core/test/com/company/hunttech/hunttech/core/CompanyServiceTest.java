    ...
    public void testBrowseLoadCompany() {
        ...
        List<Company> companies = dataManager.loadList(LoadContext.create(Company.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_Company e where e.id = :id")
    ...
    public void testSoftDeleteCompany() {
        ...
        try (Transaction tx = persistence.createTransaction()) {
            Company deleted = persistence.getEntityManager()
                    .createQuery("select e from hunttech_Company e where e.id = :id", Company.class)
                    .setParameter("id", id)
                    .getSingleResult();
            tx.commit();
        ...
        List<Company> active = dataManager.loadList(LoadContext.create(Company.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_Company e where e.id = :id and e.deleteTs is null")
                        .setParameter("id", id))
                .setView(View.LOCAL));
