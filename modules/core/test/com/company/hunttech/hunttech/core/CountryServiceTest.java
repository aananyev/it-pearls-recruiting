    ...
    public void testBrowseLoadCountry() {
        ...
        List<Country> countries = dataManager.loadList(LoadContext.create(Country.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_Country e where e.id = :id")
    ...
    public void testSoftDeleteCountry() {
        ...
        try (Transaction tx = persistence.createTransaction()) {
            Country deleted = persistence.getEntityManager()
                    .createQuery("select e from hunttech_Country e where e.id = :id", Country.class)
                    .setParameter("id", id)
                    .getSingleResult();
            tx.commit();
        ...
        List<Country> active = dataManager.loadList(LoadContext.create(Country.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_Country e where e.id = :id and e.deleteTs is null")
                        .setParameter("id", id))
                .setView(View.LOCAL));
