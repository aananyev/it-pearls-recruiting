    ...
    public void testBrowseLoadGrade() {
        ...
        List<Grade> grades = dataManager.loadList(LoadContext.create(Grade.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_Grade e where e.id = :id")
    ...
    public void testSoftDeleteGrade() {
        ...
        try (Transaction tx = persistence.createTransaction()) {
            ...
            Grade deleted = persistence.getEntityManager()
                    .createQuery("select e from hunttech_Grade e where e.id = :id", Grade.class)
                    .setParameter("id", id)
                    .getSingleResult();
            tx.commit();
        ...
        List<Grade> active = dataManager.loadList(LoadContext.create(Grade.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_Grade e where e.id = :id and e.deleteTs is null")
                        .setParameter("id", id))
                .setView(View.LOCAL));
