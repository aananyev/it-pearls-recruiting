    ...
    public void testBrowseLoadRegion() {
        ...
        List<Region> regions = dataManager.loadList(LoadContext.create(Region.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_Region e where e.id = :id")
    ...
    public void testSoftDeleteRegion() {
        ...
        try (Transaction tx = persistence.createTransaction()) {
            Region deleted = persistence.getEntityManager()
                    .createQuery("select e from hunttech_Region e where e.id = :id", Region.class)
                    .setParameter("id", id)
                    .getSingleResult();
            tx.commit();
        ...
        List<Region> active = dataManager.loadList(LoadContext.create(Region.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_Region e where e.id = :id and e.deleteTs is null")
                        .setParameter("id", id))
                .setView(View.LOCAL));
