    ...
    public void testBrowseLoadSkillTree() {
        ...
        List<SkillTree> skills = dataManager.loadList(LoadContext.create(SkillTree.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_SkillTree e where e.id = :id")
    ...
    public void testSoftDeleteSkillTree() {
        ...
        try (Transaction tx = persistence.createTransaction()) {
            SkillTree deleted = persistence.getEntityManager()
                    .createQuery("select e from hunttech_SkillTree e where e.id = :id", SkillTree.class)
                    .setParameter("id", id)
                    .getSingleResult();
            tx.commit();
        ...
        List<SkillTree> active = dataManager.loadList(LoadContext.create(SkillTree.class)
                .setQuery(LoadContext.createQuery(
                        "select e from hunttech_SkillTree e where e.id = :id and e.deleteTs is null")
                        .setParameter("id", id))
                .setView(View.LOCAL));
