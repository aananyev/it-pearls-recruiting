...
public class ExtSettingsWindow extends SettingsWindow {
    ...
    @Inject private Button aiConfigsEditBtn;
    @Inject private Button aiConfigsRemoveBtn;
    @Inject private Button aiConfigsTestBtn;
    ...
    private void initAiProfileOptions() {
        ...
        seniorityLevelField.setOptionsMap(options(
                "Junior", AiSeniorityLevel.JUNIOR,
                "Middle", AiSeniorityLevel.MIDDLE,
                "Senior", AiSeniorityLevel.SENIOR,
                "Lead", AiSeniorityLevel.LEAD,
                "Head", AiSeniorityLevel.HEAD,
                "Executive", AiSeniorityLevel.EXECUTIVE));
    ...
    private void refreshAiConfigs() {
        /*
         * Datasource привязан к extUserDs текущей сессии. Это сохраняет изоляцию
         * персональных API-подключений и исключает показ конфигураций других пользователей.
         */
    ...
    private void initAiTableActions() {
        /*
         * Операции редактирования, удаления и тестирования требуют выбранной строки.
         * Кнопки остаются выключенными, пока пользователь не выбрал конфигурацию.
         */
        refreshAiActionState();
    ...
    private void refreshAiActionState() {
        ...
        if (aiConfigsEditBtn != null) aiConfigsEditBtn.setEnabled(selected);
        if (aiConfigsRemoveBtn != null) aiConfigsRemoveBtn.setEnabled(selected);
        if (aiConfigsTestBtn != null) aiConfigsTestBtn.setEnabled(selected);
    }

    public void onAiConfigsCreateBtnClick() {
        /*
         * Редактор переиспользуется в административной карточке пользователя. Здесь
         * владелец всегда задаётся из текущей сессии, чтобы не создать чужую конфигурацию.
         */
    ...
    private void setEmailSettings() {
        ...
        smtpPasswordRequired.setValue(firstNotNull(userSettings.getSmtpPasswordRequired(), currentUser.getSmtpPasswordRequired()));
        ...
        imapPasswordRequired.setValue(firstNotNull(userSettings.getImapPasswordRequired(), currentUser.getImapPasswordRequired()));
        ...
        pop3PasswordRequired.setValue(firstNotNull(userSettings.getPop3PasswordRequired(), currentUser.getPop3PasswordRequired()));
    ...
    private boolean validateAiProfile(UserAiProfile profile) {
        ...
        if (Boolean.TRUE.equals(profile.getProfileEnabled())
            ...
            notifications.create(Notifications.NotificationType.WARNING)
                    .withCaption(getMessage("profileConsentRequired"))
                    .show();
            return false;
        }
        if (!isExperienceValid(profile.getProfessionalExperienceYears())
            ...
            notifications.create(Notifications.NotificationType.WARNING)
                    .withCaption(getMessage("experienceValidationMessage"))
                    .show();
            return false;
        }
        return true;
    ...
    @Override protected List<MenuItem> collectPermittedScreens(List<MenuItem> menuItems) { return super.collectPermittedScreens(menuItems); }
