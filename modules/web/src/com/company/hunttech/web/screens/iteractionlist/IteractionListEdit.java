...
import com.company.hunttech.web.screens.internalemailer.InternalEmailerEdit;
import com.company.hunttech.web.screens.recrutiestasks.RecrutiesTasksEdit;
...
public class IteractionListEdit extends StandardEditor<IteractionList> {
    ...
    private static final String QUERY_COUNT_BY_CANDIDATE_VACANCY =
            "select count(e) from hunttech_IteractionList e where e.candidate = :candidate and e.vacancy = :vacancy";

    private static final String QUERY_EMPLOYEE_BY_CANDIDATE =
            "select e from hunttech_Employee e where e.jobCandidate = :jobCandidate";
    ...
    private void isUnsubscribedPosition(OpenPosition op) {
        if (!askFlag) {
            ...
            Integer a = dataManager
                    .loadValue("select count(e.reacrutier) from hunttech_RecrutiesTasks e where e.reacrutier = :recrutier and e.openPosition = :openPosition and :nowDate between e.startDate and e.endDate",
                            Integer.class)
...
/*            if (newsItem.getVacancy().getSalaryMax() != null)
    ...
    private static final String QUERY_CHECK_CANDIDATE_EMPLOYEE = "select e.workStatus.inStaff from hunttech_Employee e where e.jobCandidate = :jobCandidate";
    ...
    private void sendMessages() {
        if (candidateField.getValue() != null) {
            LoadContext<SubscribeCandidateAction> loadContext = LoadContext.create(SubscribeCandidateAction.class)
                    .setQuery(LoadContext.createQuery("select e from hunttech_SubscribeCandidateAction e where e.candidate = :candidate and e.subscriber = :subscriber and :curDate between e.startDate and e.endDate")
                            .setCacheable(true)
                ...
                if (candidateField.getValue() != null && iteractionTypeField.getValue() != null) {
                    EmailInfo emailInfo = new EmailInfo(userSession.getUser().getEmail(),
                            new StringBuilder()
                                    ...
                                    .toString(), null,
                            "com/company/hunttech/templates/iteraction.html",
        ...
        try {
            if (candidateField.getValue() != null) {
                numberIteraction = dataManager
                        .loadValue("select e.numberIteraction from hunttech_IteractionList e where e.candidate = :candidate and e.numberIteraction = (select max(f.numberIteraction) from hunttech_IteractionList f where f.candidate = :candidate)", BigDecimal.class)
                        .parameter("candidate", candidateField.getValue())
                        .one();
            }
        ...
        if (numberIteraction != null) {
            ...
            try {
                oldIteraction = dataManager.load(IteractionList.class)
                        .query("select e from hunttech_IteractionList e where e.numberIteraction = :number")
    ...
    private Integer getIteractionCount() {
        ...
        try {
            if (candidateField.getValue() != null) {
                d = dataManager
                        .loadValue(
                                "select count(e) from hunttech_IteractionList e where e.candidate = :candidate",
                                Integer.class)
                        .parameter("candidate", candidateField.getValue())
                        .one();
            }
    ...
    private void copyPrevionsItems() {
        ...
        openPositionsDl
                .setQuery("select e from hunttech_OpenPosition e order by e.vacansyName");
        ...
        try {
            duplicateIteraction = dataManager.load(IteractionList.class)
                    .query("select e from hunttech_IteractionList e where e.candidate = :candidate and e.numberIteraction = (select max(f.numberIteraction) from hunttech_IteractionList f where f.candidate = :candidate)")
                    .parameter("candidate", getEditedEntity().getCandidate())
                    .view("iteractionList-edit-view")
                    .cacheable(true)
                    .one();
...
//                    description += "</ul>";
    ...
    public void addNewIteraction() {
        String classIL = "hunttech_" + getEditedEntity().getClass().getSimpleName() + ".edit";
