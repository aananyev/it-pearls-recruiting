...
import com.company.hunttech.web.screens.candidatecv.SelectRenderedImagesFromList;
...
import com.company.hunttech.web.screens.fragments.Onlytext;
import com.company.hunttech.web.screens.fragments.Skillsbar;
import com.company.hunttech.web.screens.internalemailertemplate.InternalEmailerTemplateEdit;
...
import com.company.hunttech.web.screens.jobcandidate.jobcandidatecomments.JobCandidateComment;
...
//                    strSimpleService.deleteExtraCharacters(icons.getTitleEnd() + "Action"))
    ...
    private static final String QUERY_GET_PERSONEL_RESERVE =
            "select e from hunttech_PersonelReserve e where e.jobCandidate = :jobCandidate and (e.endDate > :currDate or e.endDate is null)";
    ...
    private static final String QUERY_GET_DEFAULT_VACANCY =
            "select e from hunttech_OpenPosition e where e.vacansyName like \'Default\'";
    private final String QUERY_GET_SIGN_PERSONAL_RESERVE_PUT_INTERACTION =
            "select e from hunttech_Iteraction e where e.signPersonalReservePut = true";


    /* private BigDecimal getCountIteraction() {
        IteractionList e = dataManager.load(IteractionList.class)
                .query("select e from hunttech_IteractionList e where e.numberIteraction = (select max(f.numberIteraction) from hunttech_IteractionList f)")
...
//        String separator = separatorChar.repeat(22);
    ...
    private String jobCandidatesTableLastIteractionColumnGenerator
        ...
        Date dateValue = iteractionList != null && iteractionList.getDateIteraction() != null
                ? iteractionList.getDateIteraction()
                : event.getItem().getCreateTs();
        String date = dateValue != null ? new SimpleDateFormat("dd-MM-yyyy").format(dateValue) : null;
...
//            retStr = "button_table_green";
    ...
    private String jobCandidatesTableLastIteractionStyleProvider(JobCandidate jobCandidate) {
        ...
        if (iteractionList != null) {
            ...
            if (calendar.after(calendar1)) {
                if (iteractionList.getRecrutier() != null && !iteractionList.getRecrutier().equals(userSession.getUser())) {
    ...
    private String jobCandidatesTableLastIteractionDescriptionProvider(JobCandidate jobCandidate) {
        ...
        Date dateValue = iteractionList != null && iteractionList.getDateIteraction() != null
                ? iteractionList.getDateIteraction()
                : jobCandidate.getCreateTs();
        String formattedDate = dateValue != null ? simpleDateFormat.format(dateValue) : "";
...
/*    private IteractionList getLastIteraction(JobCandidate jobCandidate) {
                        ...
                        if (jobCandidatesTable.getSingleSelected() != null) {
                            ...
                            if (lastIteraction != null) {
                                ...
                                e.setNumberIteraction(dataManager.loadValue(
                                        "select max(e.numberIteraction) from hunttech_IteractionList e", BigDecimal.class)
    ...
    private Component findSuitableButton(JobCandidate entity) {
        if (dataManager.load(CandidateCV.class)
                .query("select e from hunttech_CandidateCV e where e.candidate = :candidate")
                .cacheable(true)
    ...
    private void createDataComponentsResume() {
        ...
        candidateCVDl.setQuery("select e from hunttech_CandidateCV e where e.candidate = :candidate order by e.datePost");
    }
    ...
    private void createDataComponents() {
        ...
        iteractionListDl.setQuery("select e from hunttech_IteractionList e where e.candidate = :candidate order by e.numberIteraction desc");
    }
    ...
    private void setStatistics(VBoxLayout statistics) {
        ...
        String QUERY_LAST_SALARY = "select e from hunttech_IteractionList e where e.iteractionType = " +
                "(select f from hunttech_Iteraction f where f.iterationName like :iteractionName) and " +
        ...
        BaseAction editAction = new BaseAction("edit")
                .withHandler(actionPerformedEvent -> {
                    screenBuilders.editor(JobCandidate.class, this)
                            .withScreenId("hunttech_JobCandidate.edit")
                            .editEntity(entity)
                            .build()
                            .show();
                })
            ...
            if (withCVCheckBox.getValue() != null) {
                if (!withCVCheckBox.getValue()) {
                    if (ratingFieldNotLower.getValue() == null) {
                        jobCandidatesDl.setQuery("select e from hunttech_JobCandidate e order by e.secondName, e.firstName");
                    } else {
                        jobCandidatesDl.setQuery(new StringBuilder()
                                .append("select e from hunttech_JobCandidate e where e in (select f.candidate from hunttech_IteractionList f where f.candidate = e) and e in (select g.candidate from hunttech_IteractionList g where g.candidate = e and g.rating >= ")
                    ...
                    if (ratingFieldNotLower.getValue() == null) {
                        jobCandidatesDl.setQuery("select e from hunttech_JobCandidate e where e in (select g.candidate from hunttech_CandidateCV g where g.candidate = e) order by e.secondName, e.firstName");
                    } else {
                        jobCandidatesDl.setQuery(new StringBuilder()
                                .append("select e from hunttech_JobCandidate e where e in (select g.candidate from hunttech_CandidateCV g where g.candidate = e) and e in (select g.candidate from hunttech_IteractionList g where g.candidate = e and g.rating >= ")
                                .append(ratingFieldNotLower.getValue().toString())
                                .append(") order by e.secondName, e.firstName")
                                .toString());
                    }
