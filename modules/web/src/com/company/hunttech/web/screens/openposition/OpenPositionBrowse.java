...
import com.company.hunttech.web.screens.openposition.openpositionfragments.OpenPositionDetailScreenFragment;
import com.company.hunttech.web.screens.openposition.openpositionviews.QuickViewOpenPositionDescription;
import com.company.hunttech.web.screens.openpositioncomment.OpenPositionCommentEdit;
import com.company.hunttech.web.screens.openpositioncomment.OpenPositionCommentsView;
import com.company.hunttech.web.screens.recrutiestasks.RecrutiesTasksGroupSubscribeBrowse;
import com.company.hunttech.web.screens.hrmasters.suggestjobcandidates.Suggestjobcandidate;
import com.company.hunttech.web.screens.simplebrowsers.JobCandidateSimpleBrowse;
import com.company.hunttech.web.screens.simplebrowsers.JobCandidateSimpleMailBrowse;
...
public class OpenPositionBrowse extends StandardLookup<OpenPosition> {
    ...
    private static final String QUERY_URGENTLY_POSITIONS =
            "select e from hunttech_OpenPosition e where e.openClose = false and e.priority >= :priority";
    private static final String QUERY_PARENT_OPENPOSITION =
            "select e from hunttech_OpenPosition e where e.parentOpenPosition = :parentOpenPosition";
    private static final String QUERY_SUBSCRIBERS_BY_POSITIONS =
            "select e from hunttech_RecrutiesTasks e where e.openPosition.id in :ids "
                    + "and e.endDate > current_date and e.closed = false";
    private static final String QUERY_CHILD_POSITIONS_BY_PARENTS =
            "select distinct e.parentOpenPosition.id from hunttech_OpenPosition e "
    ...
    private static final String QUERY_ACTIVE_RECRUITERS_COUNT_BY_POSITIONS =
            "select e.openPosition.id as openPositionId, count(e.reacrutier) as cnt from hunttech_RecrutiesTasks e "
    ...
    private static final String QUERY_AVG_RATING_BY_POSITIONS =
            "select e.openPosition.id as openPositionId, avg(e.rating) as avgRating from hunttech_OpenPositionComment e "
    ...
    private static final String QUERY_OPEN_POSITION_COMMENT_EXISTS =
            "select e.id from hunttech_OpenPosition e "
    ...
    private static final String QUERY_TEMPLATE_LETTER_EXISTS =
            "select op.id from hunttech_OpenPosition op "
    ...
    private static final String QUERY_PROJECT_DESCRIPTION_EXISTS =
            "select e.id from hunttech_Project e "
    ...
    private static final String QUERY_COMPANY_DESCRIPTION_EXISTS =
            "select e.id from hunttech_Company e "
    ...
    private void refreshBrowseLobExistsCaches(List<OpenPosition> positions) {
        ...
        if (!positionTypeIds.isEmpty()) {
            for (KeyValueEntity row : dataManager.loadValues(
                    "select e.id, e.positionEnName, e.positionRuName from hunttech_Position e where e.id in :ids")
    ...
    private String getLazyCommentText(OpenPosition openPosition) {
        ...
        if (!lazyCommentTextCache.containsKey(id)) {
            dataManager.loadValues("select e.id, e.comment from hunttech_OpenPosition e where e.id = :id")
    ...
    private String getLazyExerciseText(OpenPosition openPosition) {
        ...
        if (!lazyExerciseTextCache.containsKey(id)) {
            dataManager.loadValues("select e.id, e.exercise from hunttech_OpenPosition e where e.id = :id")
    ...
    private String getLazyMemoText(OpenPosition openPosition) {
        ...
        if (!lazyMemoTextCache.containsKey(id)) {
            dataManager.loadValues("select e.id, e.memoForInterview from hunttech_OpenPosition e where e.id = :id")
    ...
    private String getLazyProjectDescription(UUID projectId) {
        ...
        if (!lazyProjectDescriptionCache.containsKey(projectId)) {
            dataManager.loadValues("select e.id, e.projectDescription from hunttech_Project e where e.id = :id")
    ...
    private String getLazyCompanyDescription(UUID companyId) {
        ...
        if (!lazyCompanyDescriptionCache.containsKey(companyId)) {
            dataManager.loadValues("select e.id, e.companyDescription from hunttech_Company e where e.id = :id")
    ...
    private final static String QUERY_SELECT_COMMAND
            = "select e from hunttech_OpenPosition e where e.parentOpenPosition = :parentOpenPosition and e.openClose = false";
    private final static String QUERY_RECRUTIER_TASK
            = "select e from hunttech_RecrutiesTasks e where e.endDate > current_date and e.closed = false and e.openPosition = :openPosition";
    ...
    private static final String QUERY_COUNT_ITERACTIONS
            = "select e.iteractionType, count(e.iteractionType) from hunttech_IteractionList e where e.dateIteraction between :startDate and :endDate and (e.vacancy = :vacancy or (e.vacancy in (select f from hunttech_OpenPosition f where f.parentOpenPosition = :vacancy))) and e.iteractionType.statistics = true group by e.iteractionType";
    ...
    private Component findSuitableButton(OpenPosition entity) {
        ...
        if (getSubscubeOpenPosition(entity)) {
            ...
            if (dataManager.load(CandidateCV.class)
                    .query("select e from hunttech_CandidateCV e where e.resumePosition = :resumePosition")
