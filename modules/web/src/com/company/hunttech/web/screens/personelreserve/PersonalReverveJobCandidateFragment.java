package com.company.hunttech.web.screens.personelreserve;
...
@UiController("hunttech_PersonalReverveJobCandidateFragment")
...
public class PersonalReverveJobCandidateFragment extends ScreenFragment {
    ...
    private String getResumeCount() {
        String QUERY_ALL_CV = "select e from hunttech_CandidateCV e " +
    ...
    private List<IteractionList> getAllCandidateIteractions() {
        String QUERY_ALL_ITERACIONS = "select e from hunttech_IteractionList e " +
                "where e.candidate = :candidate " +
    ...
    public void setStatisticsLabel() {
        if (iteractionList.size() != 0) {
            ...
            if (iteractionList.size() > 0) {
                ...
                Label countAfterHuntTechInterview = getCountAfterHuntTechInterview();

                if (countAfterHuntTechInterview != null) {
                    statisticsHLabelBox.add(countAfterHuntTechInterview);
                }
    ...
    private Label getCountAfterHuntTechInterview() {
        Label countAfterHuntTechInterview = uiComponents.create(Label.NAME);
            ...
            if (iList.getIteractionType() != null) {
                if (iList.getIteractionType().getSignOurInterview() != null) {
                    if (iList.getIteractionType().getSignOurInterview()) {
                        ...
                        if (days != 0) {
                            countAfterHuntTechInterview.setValue("Локальное интервью: " + days + " дней");
                            countAfterHuntTechInterview.setStyleName(getStyleOnlastProcess(calendar, calendar1, calendar2));
                        } else {
                            countAfterHuntTechInterview.setValue("Сегодня локальное интервью");
                            countAfterHuntTechInterview.setStyleName("button_table_blue");
                        }

                        countAfterHuntTechInterview.setDescription("Число дней с момента последнего интервью на нашей стороне рекрутером HuntTech.");

                        return countAfterHuntTechInterview;
                    }
                }
            }
        }
