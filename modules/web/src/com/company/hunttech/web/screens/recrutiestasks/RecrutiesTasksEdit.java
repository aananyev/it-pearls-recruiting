...
public class RecrutiesTasksEdit extends StandardEditor<RecrutiesTasks> {
    ...
    private boolean isAllreadySubscribe() {
        String QUERY_GET_SUBSCRIBE = "select e from hunttech_RecrutiesTasks e " +
    ...
    public void onBeforeCommitChanges(BeforeCommitChangesEvent event) {
        SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yyyy");
        String QUERY = "select e from hunttech_RecrutiesTasks e " +
                "where e.reacrutier = :reacrutier " +
                "and e.openPosition = :openPosition " +
                "and e.closed = false " +
...
//        close(StandardOutcome.COMMIT);
    ...
    private Boolean checkSubscribePosition() {
        Integer countSubscrine = dataManager
                .loadValue("select count(e.reacrutier) from hunttech_RecrutiesTasks e " +
        ...
        if (email != null) {
            if (!email.isEmpty()) {
                emailInfo = new EmailInfo(email,
                        ...
                        null, "com/company/hunttech/templates/subscribe_to_position.html",
