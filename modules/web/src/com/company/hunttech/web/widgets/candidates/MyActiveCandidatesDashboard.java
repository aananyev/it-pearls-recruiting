package com.company.hunttech.web.widgets.candidates;
...
@UiController("hunttech_MyActiveCandidatesDashboard")
...
public class MyActiveCandidatesDashboard extends ScreenFragment {
    ...
    private final static String QUERY_MY_CANIDATE_EXCLUDE
            = "select e from hunttech_MyActiveCandidateExclude e where e.jobCandidate = :jobCandidate and e.user = :user";
    private final static String QUERY_EXCLUDE_CANDIDATES
            = "select e from hunttech_OpenPosition e where e.positionType = :positionType and not e.openClose = true and not e in (select f.vacancy from hunttech_IteractionList f where f.candidate = :candidate)";
    private static final String QUERY_CASE_CLOSED_OPEN_POSITION
            = "select e from hunttech_IteractionList e where e.iteractionType.signEndCase = true and e.candidate = :candidate";
    @Inject
...
//        projectDetailLabel.setDescription(openPosition.getVacansyName() + "<br><br>" + openPosition.getComment());
    ...
    private final static String QUERY_CASE_OPEN_POSITION
            = "select e from hunttech_IteractionList e where not (e.iteractionType.signEndCase = true) and e.candidate = :candidate and not (e.vacancy.openClose = true)";
    ...
    private static final String QUERY_OPPORTUNITY = "select e from hunttech_OpenPosition e where e.positionType = :positionType and not e.openClose = true";
