package com.company.hunttech.web.widgets.reports;
...
@UiController("hunttech_FunnelHuntingWidget")
@UiDescriptor("funnel-hunting-widget.xml")
@DashboardWidget(name = "Воронка хантинга")
public class FunnelHuntingWidget extends ScreenFragment {
    ...
    private static final String QUERY_BULK_COUNTS =
            "select e.recrutier.id as recrutierId, it.iterationName as iteractionName, count(e) as cnt " +
                    "from hunttech_IteractionList e join e.iteractionType it " +
    ...
    private String ITRKT_ASSIGN_ITPEARKS_INTERVIEW = "Назначено собеседование с рекрутером HuntTech";
    private String ITRKT_PREPARE_ITPEARKS_INTERVIEW = "Прошел собеседование с рекрутером HuntTech";
...
//        widgetTitle.setValue(title + df.format(startDate) + " - " + df.format(endDate));
    ...
    private void getResearchersList() {
        reaearchers = dataManager.load(User.class)
                ...
                .cacheable(true)
                .parameter("groupNames", Arrays.asList(RESEARCHER, RESEARCHER_INTERN))
                .list();
