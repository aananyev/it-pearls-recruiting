...
-- -----------------------------------------------------------------------------
-- 1. HUNTTECH_OPEN_POSITION — AI / текстовые поля (OpenPosition entity)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2. HUNTTECH_USER_AI_CONFIGURATION (UserAiConfiguration entity)
-- -----------------------------------------------------------------------------
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'fk_hunttech_user_ai_configuration_on_user'
    ) THEN
...
-- -----------------------------------------------------------------------------
-- 3. HUNTTECH_VACANCY_PROMPT_TEMPLATE (VacancyPromptTemplate entity)
-- -----------------------------------------------------------------------------
...
UPDATE SEC_USER
SET DTYPE = 'hunttech_ExtUser'
WHERE DTYPE = 'itpearls_ExtUser';

UPDATE DASHBOARD_PERSISTENT_DASHBOARD
SET DASHBOARD_MODEL = replace(DASHBOARD_MODEL, 'itpearls_', 'hunttech_')
WHERE DASHBOARD_MODEL LIKE '%itpearls_%';
