...
**БД:** `HuntTech` @ `127.0.0.1:5432`, пользователь `cuba`  
**Источник ожидаемой схемы:** `modules/global/src/com/company/hunttech/entity/`, `persistence.xml`, `modules/core/db/update/postgres/`
...
> **Локальная БД `HuntTech`:** после проверки синтаксиса скрипта структура приведена в соответствие с entity (колонки `SEC_USER`, `HUNTTECH_OPEN_POSITION`, таблицы AI). Записей в `SYS_DB_CHANGELOG` для `260627`/`260629` по-прежнему нет — на других стендах используйте `./gradlew updateDb` или идемпотентный SQL ниже.
...
| `HUNTTECH_OPEN_POSITION` | `RAW_DESCRIPTION`, `INTERVIEW_CHECKLIST`, `SEARCH_MAP`, `INTERVIEW_PLAN` | колонки отсутствуют | AI-поля вакансии, экран OpenPosition Edit |
| `HUNTTECH_USER_AI_CONFIGURATION` | таблица целиком | таблица отсутствует | настройки AI пользователя |
| `HUNTTECH_VACANCY_PROMPT_TEMPLATE` | таблица целиком | таблица отсутствует | шаблоны промптов для вакансий |

## В синхронизации (не требуют миграции)

| Объект | Примечание |
|--------|------------|
| `HUNTTECH_USER_SETTINGS` | `IMAGE_ID` (fileImageFace) — **есть**, соответствует entity |
| `HUNTTECH_LABOR_AGREEMENT` | все колонки entity присутствуют |
| `HUNTTECH_JOB_CANDIDATE` | `FILE_IMAGE_FACE` / связанные поля — в порядке |
| Join-таблицы (`HUNTTECH_OPEN_POSITION_LABOR_AGREEMENT_LINK` и др.) | существуют |
...
- `DTYPE` в `HUNTTECH_OPEN_POSITION` — legacy в БД, не объявлен в entity (не блокер).
