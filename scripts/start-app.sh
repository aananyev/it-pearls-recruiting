...
# Gradle's Tomcat task may launch Catalina from a process that does not inherit
# the profile environment. Export the resolved datasource values in the ignored
# runtime setenv file so Tomcat can resolve ${env.*} placeholders consistently.
configure_tomcat_db_environment() {
  local setenv="$ROOT/deploy/tomcat/bin/setenv.sh"
  local profile_password="${!DB_PROFILE_PASSWORD_VAR}"
  printf '\n# Generated by start-app.sh for the selected database profile.\n' >> "$setenv"
  printf 'export HUNTTECH_DB_PASSWORD=%q\n' "$profile_password" >> "$setenv"
  printf 'export HUNTTECH_DB_JDBC_URL=%q\n' "$(db_profile_jdbc_url)" >> "$setenv"
  printf 'export HUNTTECH_DB_USER=%q\n' "$DB_PROFILE_USER" >> "$setenv"
}
configure_tomcat_db_environment
